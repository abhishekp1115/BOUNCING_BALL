...
function handleRings() {
  for (var i = 0; i < rings.length; i++) {
    ...
    if (overlaped) {
      rings.splice(i, 1); // deleting the ring 
...
function showMessages() {
  textSize(23);
  stroke(15); // border line 4 the text
  fill("black");
  if (gameState === "initial") {
  ...
  if (gameState === "win") {
    text("WOW YOU WIN!", width / 3, height / 2);
  }
  if (gameState === "end") {
...
function displayBricks() {
  ...
  if (allbricks[3].body.position.x < -1300) {
    push(); // USED TO CUSTOMIZE THE OBJECT 
    imageMode(CENTER);
    image(homeImg, 900, 380, 210, 150);
    pop(); // USED TO CUSTOMIZE THE OBJECT 
  }
}